...
const Navbar = () => {
  const navItems = [
    { text: "ESP/ENG", link: "#" },
    { text: "FAQ", link: "#" },
    { text: "nuestra misión", link: "#" },
    { text: "mi cuenta", link: "#", icon: "fa-regular fa-user" },
  ];
        ...
        <nav className="flex justify-between items-center py-4 border-b-4 border-black">
          ...
          <ul className="flex space-x-4 place-self-end text-lg">
            {navItems.map((item, index) => (
              <li
                key={index}
                className={`relative ${index < navItems.length - 1 ? "after:content-['·'] after:absolute after:right-[-12px] after:top-1/2 after:transform after:-translate-y-1/2 after:text-lg after:text-black" : ""}`}
              >
        ...
        <header className="text-center my-12 flex flex-col md:flex-row justify-between md:mx-48">
          <div className="">
            <p className="mt-4 text-3xl text-left">
              ...
              <br /> forma más fácil, barata y
              <br /> sostenible alquilando nuestros
